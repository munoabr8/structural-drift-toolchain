...
heap_under_mb_darwin() {
  ...
  awk -v cap="$cap_mb" -v want="$col_name" '
    ...
    /^REGION TYPE/ && !hdr_seen { hdr_seen=1; next }          # header title line
    hdr_seen && !col && NF {                                   # next non-empty = columns
      for(i=1;i<=NF;i++) if($i==want) col=i; if(!col){exit 2}  # indeterminate if col missing
      inside=1; next
    }
    inside && NF==0 { next }                                   # skip blank separators
    inside && $1 ~ /^MALLOC/ && col>0 {
